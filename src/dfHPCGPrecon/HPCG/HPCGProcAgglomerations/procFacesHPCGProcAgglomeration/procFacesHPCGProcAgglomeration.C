...
#include "procFacesHPCGProcAgglomeration.H"
#include "addToRunTimeSelectionTable.H"
#include "HPCGAgglomeration.H"
#include "Random.H"
#include "lduMesh.H"
#include "processorLduInterface.H"
#include "processorHPCGInterface.H"
...
namespace Foam
{
    defineTypeNameAndDebug(procFacesHPCGProcAgglomeration, 0);

    addToRunTimeSelectionTable
    (
        HPCGProcAgglomeration,
        procFacesHPCGProcAgglomeration,
        HPCGAgglomeration
    );
}
...
Foam::procFacesHPCGProcAgglomeration::singleCellMesh
(
...
Foam::procFacesHPCGProcAgglomeration::processorAgglomeration
(
    const lduMesh& mesh
) const
{
    ...
    if (singleCellMeshPtr.valid())
    {
        ...
        label nCoarseProcs;
        fineToCoarse = pairHPCGAgglomeration::agglomerate
        (
            nCoarseProcs,
            singleCellMesh,
            faceWeights
...
bool Foam::procFacesHPCGProcAgglomeration::doProcessorAgglomeration
(
    const lduMesh& mesh
) const
{
...
Foam::procFacesHPCGProcAgglomeration::procFacesHPCGProcAgglomeration
(
    HPCGAgglomeration& agglom,
    const dictionary& controlDict
)
:
    HPCGProcAgglomeration(agglom, controlDict),
...
bool Foam::procFacesHPCGProcAgglomeration::agglomerate()
{
    if (debug)
    {
        ...
        )
        {
            if (agglom_.hasMeshLevel(fineLevelIndex))
            {
                ...
                if (nProcs > 1 && doProcessorAgglomeration(levelMesh))
                {
                    ...
                    HPCGAgglomeration::calculateRegionMaster
                    (
                        levelComm,
                        procAgglomMap,
                        masterProcs,
                    ...
                    // collecting.
                    HPCGProcAgglomeration::agglomerate
                    (
                        fineLevelIndex,
                        procAgglomMap,
                        masterProcs,
