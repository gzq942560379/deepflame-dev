...
namespace Foam
{
    defineTypeNameAndDebug(HPCGProcAgglomeration, 0);
    defineRunTimeSelectionTable(HPCGProcAgglomeration, HPCGAgglomeration);
}


// * * * * * * * * * * * * Protected Member Functions  * * * * * * * * * * * //

void Foam::HPCGProcAgglomeration::printStats
(
    Ostream& os,
...
Foam::labelListList Foam::HPCGProcAgglomeration::globalCellCells
(
    const lduMesh& mesh
)
{
...
bool Foam::HPCGProcAgglomeration::agglomerate
(
...
Foam::HPCGProcAgglomeration::HPCGProcAgglomeration
(
    HPCGAgglomeration& agglom,
...
Foam::autoPtr<Foam::HPCGProcAgglomeration> Foam::HPCGProcAgglomeration::New
(
    const word& type,
    HPCGAgglomeration& agglom,
    const dictionary& controlDict
)
{
    if (debug)
    {
        InfoInFunction << "Constructing HPCGProcAgglomeration" << endl;
    }

    HPCGAgglomerationConstructorTable::iterator cstrIter =
        HPCGAgglomerationConstructorTablePtr_->find(type);

    if (cstrIter == HPCGAgglomerationConstructorTablePtr_->end())
    {
        FatalErrorInFunction
            << "Unknown HPCGProcAgglomeration type "
            << type << " for HPCGAgglomeration " << agglom.type() << nl << nl
            << "Valid HPCGProcAgglomeration types are :" << endl
            << HPCGAgglomerationConstructorTablePtr_->sortedToc()
            << exit(FatalError);
    }

    return autoPtr<HPCGProcAgglomeration>(cstrIter()(agglom, controlDict));
}
