...
void Foam::HPCGSolver::agglomerateMatrix
(
...
void Foam::HPCGSolver::agglomerateInterfaceCoefficients
(
    const label fineLevelIndex,
    const lduInterfacePtrsList& coarseMeshInterfaces,
    PtrList<lduInterfaceField>& coarsePrimInterfaces,
...
) const
{
    ...
    forAll(fineInterfaces, inti)
    {
        ...
        {
            const HPCGInterface& coarseInterface =
...
void Foam::HPCGSolver::gatherMatrices
(
...
) const
{
    if (debug)
    {
        Pout<< "HPCGSolver::gatherMatrices :"
...
void Foam::HPCGSolver::procAgglomerateMatrix
(
            ...
            forAll(bMap, procIntI)
            {
                ...
                if (allIntI != -1)
                {
                    ...
                    if (!allInterfaces.set(allIntI))
                    {
                        ...
                        if (proci == 0)
                        {
                            const processorHPCGInterfaceField& procInt =
                                refCast
                                <
                                    const processorHPCGInterfaceField
                                >
                                (
