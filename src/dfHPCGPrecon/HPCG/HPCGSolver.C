...
namespace Foam
{
    defineTypeNameAndDebug(HPCGSolver, 0);

    lduMatrix::solver::addsymMatrixConstructorToTable<HPCGSolver>
        addHPCGSolverMatrixConstructorToTable_;

    lduMatrix::solver::addasymMatrixConstructorToTable<HPCGSolver>
        addHPCGAsymSolverMatrixConstructorToTable_;
}


// * * * * * * * * * * * * * * * * Constructors  * * * * * * * * * * * * * * //

Foam::HPCGSolver::HPCGSolver
(
...
:
    ...
    agglomeration_(HPCGAgglomeration::New(matrix_, controlDict_)),
    smoothCoarsestLevel_(false),
    solveCoarsestLevel_(true),
    ...
    smoothsolver_(false)
{
        ...
        forAll(agglomeration_, fineLevelIndex)
        {
            if (agglomeration_.hasMeshLevel(fineLevelIndex))
            {
                ...
                )
                {
                    ...
                    PtrList<HPCGInterface> dummyPrimMeshInterfaces
                    (
    ...
    else
    {
        ...
        //Info << "global Cell" << globalCells.size() << endl;
    }
    ...
    else
    {
        FatalErrorInFunction
            << "No coarse levels created, either matrix too small for HPCG"
...
void Foam::HPCGSolver::readControls()
{
    ...
    controlDict_.readIfPresent("smoothCoarsestLevel", smoothCoarsestLevel_);
    controlDict_.readIfPresent("solveCoarsestLevel", solveCoarsestLevel_);
    controlDict_.readIfPresent("smoothsolver", smoothsolver_);

    if (debug)
    {
        Pout<< "HPCGSolver settings :"
...
const Foam::lduMatrix& Foam::HPCGSolver::matrixLevel(const label i) const
{
    if (i == 0)
    {
        return matrix_;
...
const Foam::lduInterfaceFieldPtrsList& Foam::HPCGSolver::interfaceLevel
(
    const label i
) const
{
...
Foam::HPCGSolver::interfaceBouCoeffsLevel
(
    const label i
) const
{
...
const Foam::FieldField<Foam::Field, Foam::scalar>&
Foam::HPCGSolver::interfaceIntCoeffsLevel
(
    const label i
) const
{
