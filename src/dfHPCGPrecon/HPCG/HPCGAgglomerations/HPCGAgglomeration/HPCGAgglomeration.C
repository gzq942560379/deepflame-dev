...
#include "HPCGAgglomeration.H"
#include "lduMesh.H"
#include "lduMatrix.H"
#include "Time.H"
#include "HPCGInterface.H"
#include "HPCGProcAgglomeration.H"
#include "pairHPCGAgglomeration.H"
#include "IOmanip.H"
...
namespace Foam
{
    defineTypeNameAndDebug(HPCGAgglomeration, 0);
    defineRunTimeSelectionTable(HPCGAgglomeration, lduMesh);
    defineRunTimeSelectionTable(HPCGAgglomeration, lduMatrix);
    defineRunTimeSelectionTable(HPCGAgglomeration, geometry);
}


// * * * * * * * * * * * * Protected Member Functions  * * * * * * * * * * * //

void Foam::HPCGAgglomeration::compactLevels(const label nCreatedLevels)
{
...
bool Foam::HPCGAgglomeration::continueAgglomerating
(
...
Foam::HPCGAgglomeration::HPCGAgglomeration
(
    const lduMesh& mesh,
    const dictionary& controlDict
)
:
    MeshObject<lduMesh, Foam::GeometricMeshObject, HPCGAgglomeration>(mesh),
    ...
    (
      ...
      ? HPCGProcAgglomeration::New
        (
            controlDict.lookup("processorAgglomerator"),
            *this,
            controlDict
        )
      : autoPtr<HPCGProcAgglomeration>(nullptr)
    ),
...
const Foam::HPCGAgglomeration& Foam::HPCGAgglomeration::New
(
    const lduMesh& mesh,
    const dictionary& controlDict
)
{
    if
    (
        !mesh.thisDb().foundObject<HPCGAgglomeration>
        (
            HPCGAgglomeration::typeName
        )
    )
    {
        ...
        (
            controlDict,
            "geometricHPCGAgglomerationLibs",
        ...
        if (cstrIter == lduMeshConstructorTablePtr_->end())
        {
            FatalErrorInFunction
                << "Unknown HPCGAgglomeration type "
                << agglomeratorType << ".\n"
                << "Valid matrix HPCGAgglomeration types are :"
                << lduMatrixConstructorTablePtr_->sortedToc() << endl
                << "Valid geometric HPCGAgglomeration types are :"
    ...
    {
        return mesh.thisDb().lookupObject<HPCGAgglomeration>
        (
            HPCGAgglomeration::typeName
        );
    }
...
)
{
    ...
    )
    {
        ...
        (
            controlDict,
            "algebraicHPCGAgglomerationLibs",
...
Foam::autoPtr<Foam::HPCGAgglomeration> Foam::HPCGAgglomeration::New
(
    const lduMesh& mesh,
...
)
{
    ...
    return autoPtr<HPCGAgglomeration>
    (
        cstrIter()
        (
            mesh,
...
const Foam::lduMesh& Foam::HPCGAgglomeration::meshLevel
(
    const label i
) const
{
...
bool Foam::HPCGAgglomeration::hasMeshLevel(const label i) const
{
    if (i == 0)
    {
        return true;
...
const Foam::lduInterfacePtrsList& Foam::HPCGAgglomeration::interfaceLevel
(
    const label i
) const
{
...
void Foam::HPCGAgglomeration::clearLevel(const label i)
{
    if (hasMeshLevel(i))
    {
...
const Foam::labelList& Foam::HPCGAgglomeration::procAgglomMap
(
    const label leveli
) const
{
...
const Foam::labelList& Foam::HPCGAgglomeration::agglomProcIDs
(
    const label leveli
) const
{
...
bool Foam::HPCGAgglomeration::hasProcMesh(const label leveli) const
{
    return procCommunicator_[leveli] != -1;
}


Foam::label Foam::HPCGAgglomeration::procCommunicator(const label leveli) const
{
    return procCommunicator_[leveli];
}


const Foam::labelList& Foam::HPCGAgglomeration::cellOffsets
(
    const label leveli
) const
{
...
const Foam::labelListList& Foam::HPCGAgglomeration::faceMap
(
    const label leveli
) const
{
...
const Foam::labelListList& Foam::HPCGAgglomeration::boundaryMap
(
    const label leveli
) const
{
...
const Foam::labelListListList& Foam::HPCGAgglomeration::boundaryFaceMap
(
    const label leveli
) const
{
...
bool Foam::HPCGAgglomeration::checkRestriction
(
