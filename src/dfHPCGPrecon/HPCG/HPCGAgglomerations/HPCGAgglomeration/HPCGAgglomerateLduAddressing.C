...
void Foam::HPCGAgglomeration::agglomerateLduAddressing
(
...
)
{
    ...
    forAll(fineInterfaces, inti)
    {
        if (fineInterfaces.set(inti))
        {
            ...
            patchFineToCoarse[inti] = refCast<const HPCGInterface>
            (
    ...
    (
        coarseInterfaces,
        lduPrimitiveMesh::nonBlockingSchedule<processorHPCGInterface>
        (
            coarseInterfaces
        )
    );
...
void Foam::HPCGAgglomeration::procAgglomerateLduAddressing
(
...
void Foam::HPCGAgglomeration::procAgglomerateRestrictAddressing
(
    const label comm,
    const labelList& procIDs,
    const label levelIndex
...
void Foam::HPCGAgglomeration::combineLevels(const label curLevel)
{
    ...
    forAll(prevInterLevel, inti)
    {
        ...
        {
            HPCGInterface& prevInt = refCast<HPCGInterface>
            (
            ...
            const HPCGInterface& curInt = refCast<const HPCGInterface>
            (
                curInterLevel[inti]
            );
...
//void Foam::HPCGAgglomeration::gatherList
//(
//    const label comm,
//    const labelList& procIDs,
//
...
void Foam::HPCGAgglomeration::calculateRegionMaster
(
    const label comm,
    const labelList& procAgglomMap,
