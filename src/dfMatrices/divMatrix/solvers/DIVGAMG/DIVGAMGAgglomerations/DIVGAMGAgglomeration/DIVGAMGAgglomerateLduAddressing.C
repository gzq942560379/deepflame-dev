...
#include "processorDIVGAMGInterface.H"

// * * * * * * * * * * * * * * * Member Functions  * * * * * * * * * * * * * //

void Foam::DIVGAMGAgglomeration::agglomerateLduAddressing
(
...
)
{
    ...
    forAll(fineInterfaces, inti)
    {
        if (fineInterfaces.set(inti))
        {
            ...
            patchFineToCoarse[inti] = refCast<const DIVGAMGInterface>
            (
    ...
    (
        coarseInterfaces,
        lduPrimitiveMesh::nonBlockingSchedule<processorDIVGAMGInterface>
        (
            coarseInterfaces
        )
    );
...
void Foam::DIVGAMGAgglomeration::procAgglomerateLduAddressing
(
...
void Foam::DIVGAMGAgglomeration::procAgglomerateRestrictAddressing
(
    const label comm,
    const labelList& procIDs,
    const label levelIndex
...
void Foam::DIVGAMGAgglomeration::combineLevels(const label curLevel)
{
    ...
    forAll(prevInterLevel, inti)
    {
        ...
        {
            DIVGAMGInterface& prevInt = refCast<DIVGAMGInterface>
            (
            ...
            const DIVGAMGInterface& curInt = refCast<const DIVGAMGInterface>
            (
                curInterLevel[inti]
            );
...
//void Foam::DIVGAMGAgglomeration::gatherList
//(
//    const label comm,
//    const labelList& procIDs,
//
...
void Foam::DIVGAMGAgglomeration::calculateRegionMaster
(
    const label comm,
    const labelList& procAgglomMap,
