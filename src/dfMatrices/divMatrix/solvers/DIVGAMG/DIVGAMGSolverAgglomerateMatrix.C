...
#include "DIVGAMGInterfaceField.H"
#include "processorLduInterfaceField.H"
#include "processorDIVGAMGInterfaceField.H"

// * * * * * * * * * * * * * Private Member Functions  * * * * * * * * * * * //

void Foam::DIVGAMGSolver::agglomerateMatrix
(
...
void Foam::DIVGAMGSolver::agglomerateInterfaceCoefficients
(
    const label fineLevelIndex,
    const lduInterfacePtrsList& coarseMeshInterfaces,
    PtrList<lduInterfaceField>& coarsePrimInterfaces,
...
) const
{
    ...
    forAll(fineInterfaces, inti)
    {
        ...
        {
            const DIVGAMGInterface& coarseInterface =
...
void Foam::DIVGAMGSolver::gatherMatrices
(
...
) const
{
    if (debug)
    {
        Pout<< "DIVGAMGSolver::gatherMatrices :"
...
void Foam::DIVGAMGSolver::procAgglomerateMatrix
(
            ...
            forAll(bMap, procIntI)
            {
                ...
                if (allIntI != -1)
                {
                    ...
                    if (!allInterfaces.set(allIntI))
                    {
                        ...
                        if (proci == 0)
                        {
                            const processorDIVGAMGInterfaceField& procInt =
                                refCast
                                <
                                    const processorDIVGAMGInterfaceField
                                >
                                (
