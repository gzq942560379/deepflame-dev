...
namespace Foam
{
    defineTypeNameAndDebug(DIVGAMGSolver, 0);

    divMatrix::solver::addsymMatrixConstructorToTable<DIVGAMGSolver>
        addDIVGAMGSolverMatrixConstructorToTable_;

    divMatrix::solver::addasymMatrixConstructorToTable<DIVGAMGSolver>
        addDIVGAMGAsymSolverMatrixConstructorToTable_;
}


// * * * * * * * * * * * * * * * * Constructors  * * * * * * * * * * * * * * //

Foam::DIVGAMGSolver::DIVGAMGSolver
(
    ...
    interfaceLevelsIntCoeffs_(agglomeration_.size())
{
    ...
    Info << "DIVGAMGSolver settings :" << endl
        ...
        forAll(agglomeration_, fineLevelIndex)
        {
            if (agglomeration_.hasMeshLevel(fineLevelIndex))
            {
                ...
                )
                {
                    ...
                    PtrList<DIVGAMGInterface> dummyPrimMeshInterfaces
                    (
                    ...
                    forAll(fineMeshInterfaces, intI)
                    {
                        ...
                        {
                            OStringStream os;
                            refCast<const DIVGAMGInterface>
                            (
...
void Foam::DIVGAMGSolver::readControls()
{
    divMatrix::solver::readControls();
    ...
    if (debug)
    {
        Pout<< "DIVGAMGSolver settings :" << endl
            << "    cacheAgglomeration:" << cacheAgglomeration_ << endl
            << "    nPreSweeps:" << nPreSweeps_ << endl
            << "    preSweepsLevelMultiplier:" << preSweepsLevelMultiplier_ << endl
            << "    maxPreSweeps:" << maxPreSweeps_ << endl
...
const Foam::lduMatrix& Foam::DIVGAMGSolver::matrixLevel(const label i) const
{
    if (i == 0)
    {
        return matrix_.ldu();
...
Foam::DIVGAMGSolver::interfaceBouCoeffsLevel
(
    const label i
) const
{
...
const Foam::FieldField<Foam::Field, Foam::scalar>&
Foam::DIVGAMGSolver::interfaceIntCoeffsLevel
(
    const label i
) const
{
