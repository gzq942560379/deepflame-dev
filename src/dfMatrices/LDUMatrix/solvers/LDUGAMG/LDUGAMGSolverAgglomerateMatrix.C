...
void Foam::LDUGAMGSolver::agglomerateMatrix
(
...
)
{
    ...
    if (UPstream::myProcNo(fineMatrix.mesh().comm()) != -1)
    {
        ...
        // Set the coarse level matrix
        LDUMatrixLevels_.set
        (
            fineLevelIndex,
            new LDUMatrix(coarseMatrix)
        );
...
void Foam::LDUGAMGSolver::agglomerateInterfaceCoefficients
(
    const label fineLevelIndex,
    const lduInterfacePtrsList& coarseMeshInterfaces,
    PtrList<lduInterfaceField>& coarsePrimInterfaces,
...
) const
{
    ...
    forAll(fineInterfaces, inti)
    {
        ...
        {
            const LDUGAMGInterface& coarseInterface =
...
void Foam::LDUGAMGSolver::gatherMatrices
(
...
) const
{
    if (debug)
    {
        Pout<< "LDUGAMGSolver::gatherMatrices :"
...
void Foam::LDUGAMGSolver::procAgglomerateMatrix
(
            ...
            forAll(bMap, procIntI)
            {
                ...
                if (allIntI != -1)
                {
                    ...
                    if (!allInterfaces.set(allIntI))
                    {
                        ...
                        if (proci == 0)
                        {
                            const processorLDUGAMGInterfaceField& procInt =
                                refCast
                                <
                                    const processorLDUGAMGInterfaceField
                                >
                                (
