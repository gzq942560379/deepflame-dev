...
namespace Foam
{
    defineTypeNameAndDebug(LDUGAMGProcAgglomeration, 0);
    defineRunTimeSelectionTable(LDUGAMGProcAgglomeration, LDUGAMGAgglomeration);
}


// * * * * * * * * * * * * Protected Member Functions  * * * * * * * * * * * //

void Foam::LDUGAMGProcAgglomeration::printStats
(
    Ostream& os,
...
Foam::labelListList Foam::LDUGAMGProcAgglomeration::globalCellCells
(
    const lduMesh& mesh
)
{
...
bool Foam::LDUGAMGProcAgglomeration::agglomerate
(
...
Foam::LDUGAMGProcAgglomeration::LDUGAMGProcAgglomeration
(
    LDUGAMGAgglomeration& agglom,
...
Foam::autoPtr<Foam::LDUGAMGProcAgglomeration> Foam::LDUGAMGProcAgglomeration::New
(
    const word& type,
    LDUGAMGAgglomeration& agglom,
    const dictionary& controlDict
)
{
    if (debug)
    {
        InfoInFunction << "Constructing LDUGAMGProcAgglomeration" << endl;
    }

    LDUGAMGAgglomerationConstructorTable::iterator cstrIter =
        LDUGAMGAgglomerationConstructorTablePtr_->find(type);

    if (cstrIter == LDUGAMGAgglomerationConstructorTablePtr_->end())
    {
        FatalErrorInFunction
            << "Unknown LDUGAMGProcAgglomeration type "
            << type << " for LDUGAMGAgglomeration " << agglom.type() << nl << nl
            << "Valid LDUGAMGProcAgglomeration types are :" << endl
            << LDUGAMGAgglomerationConstructorTablePtr_->sortedToc()
            << exit(FatalError);
    }

    return autoPtr<LDUGAMGProcAgglomeration>(cstrIter()(agglom, controlDict));
}
