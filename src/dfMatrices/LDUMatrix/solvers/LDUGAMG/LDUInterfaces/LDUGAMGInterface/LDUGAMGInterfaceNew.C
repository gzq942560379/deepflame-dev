...
Foam::autoPtr<Foam::LDUGAMGInterface> Foam::LDUGAMGInterface::New
(
    const label index,
...
)
{
    ...
    if (cstrIter == lduInterfaceConstructorTablePtr_->end())
    {
        FatalErrorInFunction
            << "Unknown LDUGAMGInterface type " << coupleType << ".\n"
            << "Valid LDUGAMGInterface types are :"
            << lduInterfaceConstructorTablePtr_->sortedToc()
            << exit(FatalError);
    }

    return autoPtr<LDUGAMGInterface>
    (
        cstrIter()
        (
            index,
...
)
{
    ...
    return autoPtr<LDUGAMGInterface>(cstrIter()(index, coarseInterfaces, is));
}
