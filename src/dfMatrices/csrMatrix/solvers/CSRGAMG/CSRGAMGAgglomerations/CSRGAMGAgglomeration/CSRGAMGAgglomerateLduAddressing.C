...
#include "processorCSRGAMGInterface.H"

// * * * * * * * * * * * * * * * Member Functions  * * * * * * * * * * * * * //

void Foam::CSRGAMGAgglomeration::agglomerateLduAddressing
(
...
)
{
    ...
    forAll(fineInterfaces, inti)
    {
        if (fineInterfaces.set(inti))
        {
            ...
            patchFineToCoarse[inti] = refCast<const CSRGAMGInterface>
            (
    ...
    (
        coarseInterfaces,
        lduPrimitiveMesh::nonBlockingSchedule<processorCSRGAMGInterface>
        (
            coarseInterfaces
        )
    );
...
void Foam::CSRGAMGAgglomeration::procAgglomerateLduAddressing
(
...
void Foam::CSRGAMGAgglomeration::procAgglomerateRestrictAddressing
(
    const label comm,
    const labelList& procIDs,
    const label levelIndex
...
void Foam::CSRGAMGAgglomeration::combineLevels(const label curLevel)
{
    ...
    forAll(prevInterLevel, inti)
    {
        ...
        {
            CSRGAMGInterface& prevInt = refCast<CSRGAMGInterface>
            (
            ...
            const CSRGAMGInterface& curInt = refCast<const CSRGAMGInterface>
            (
                curInterLevel[inti]
            );
...
//void Foam::CSRGAMGAgglomeration::gatherList
//(
//    const label comm,
//    const labelList& procIDs,
//
...
void Foam::CSRGAMGAgglomeration::calculateRegionMaster
(
    const label comm,
    const labelList& procAgglomMap,
