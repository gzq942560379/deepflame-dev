...
#include "CSRGAMGInterfaceField.H"
#include "processorLduInterfaceField.H"
#include "processorCSRGAMGInterfaceField.H"

// * * * * * * * * * * * * * Private Member Functions  * * * * * * * * * * * //

void Foam::CSRGAMGSolver::agglomerateMatrix
(
...
void Foam::CSRGAMGSolver::agglomerateInterfaceCoefficients
(
    const label fineLevelIndex,
    const lduInterfacePtrsList& coarseMeshInterfaces,
    PtrList<lduInterfaceField>& coarsePrimInterfaces,
...
) const
{
    ...
    forAll(fineInterfaces, inti)
    {
        ...
        {
            const CSRGAMGInterface& coarseInterface =
...
void Foam::CSRGAMGSolver::gatherMatrices
(
...
) const
{
    if (debug)
    {
        Pout<< "CSRGAMGSolver::gatherMatrices :"
...
void Foam::CSRGAMGSolver::procAgglomerateMatrix
(
            ...
            forAll(bMap, procIntI)
            {
                ...
                if (allIntI != -1)
                {
                    ...
                    if (!allInterfaces.set(allIntI))
                    {
                        ...
                        if (proci == 0)
                        {
                            const processorCSRGAMGInterfaceField& procInt =
                                refCast
                                <
                                    const processorCSRGAMGInterfaceField
                                >
                                (
