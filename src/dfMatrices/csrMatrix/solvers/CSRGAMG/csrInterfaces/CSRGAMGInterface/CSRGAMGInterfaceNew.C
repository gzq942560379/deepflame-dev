...
Foam::autoPtr<Foam::CSRGAMGInterface> Foam::CSRGAMGInterface::New
(
    const label index,
...
)
{
    ...
    if (cstrIter == lduInterfaceConstructorTablePtr_->end())
    {
        FatalErrorInFunction
            << "Unknown CSRGAMGInterface type " << coupleType << ".\n"
            << "Valid CSRGAMGInterface types are :"
            << lduInterfaceConstructorTablePtr_->sortedToc()
            << exit(FatalError);
    }

    return autoPtr<CSRGAMGInterface>
    (
        cstrIter()
        (
            index,
...
)
{
    ...
    return autoPtr<CSRGAMGInterface>(cstrIter()(index, coarseInterfaces, is));
}
