...
namespace Foam
{
    defineTypeNameAndDebug(CSRGAMGProcAgglomeration, 0);
    defineRunTimeSelectionTable(CSRGAMGProcAgglomeration, CSRGAMGAgglomeration);
}


// * * * * * * * * * * * * Protected Member Functions  * * * * * * * * * * * //

void Foam::CSRGAMGProcAgglomeration::printStats
(
    Ostream& os,
    CSRGAMGAgglomeration& agglom
) const
{
...
Foam::labelListList Foam::CSRGAMGProcAgglomeration::globalCellCells
(
    const lduMesh& mesh
)
{
...
bool Foam::CSRGAMGProcAgglomeration::agglomerate
(
...
Foam::CSRGAMGProcAgglomeration::CSRGAMGProcAgglomeration
(
    CSRGAMGAgglomeration& agglom,
...
Foam::autoPtr<Foam::CSRGAMGProcAgglomeration> Foam::CSRGAMGProcAgglomeration::New
(
    const word& type,
    CSRGAMGAgglomeration& agglom,
    const dictionary& controlDict
)
{
    if (debug)
    {
        InfoInFunction << "Constructing CSRGAMGProcAgglomeration" << endl;
    }

    CSRGAMGAgglomerationConstructorTable::iterator cstrIter =
        CSRGAMGAgglomerationConstructorTablePtr_->find(type);

    if (cstrIter == CSRGAMGAgglomerationConstructorTablePtr_->end())
    {
        FatalErrorInFunction
            << "Unknown CSRGAMGProcAgglomeration type "
            << type << " for CSRGAMGAgglomeration " << agglom.type() << nl << nl
            << "Valid CSRGAMGProcAgglomeration types are :" << endl
            << CSRGAMGAgglomerationConstructorTablePtr_->sortedToc()
            << exit(FatalError);
    }

    return autoPtr<CSRGAMGProcAgglomeration>(cstrIter()(agglom, controlDict));
}
