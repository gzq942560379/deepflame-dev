...
Foam::autoPtr<Foam::dfGAMGInterface> Foam::dfGAMGInterface::New
(
    const label index,
...
)
{
    ...
    if (cstrIter == lduInterfaceConstructorTablePtr_->end())
    {
        FatalErrorInFunction
            << "Unknown dfGAMGInterface type " << coupleType << ".\n"
            << "Valid dfGAMGInterface types are :"
            << lduInterfaceConstructorTablePtr_->sortedToc()
            << exit(FatalError);
    }

    return autoPtr<dfGAMGInterface>
    (
        cstrIter()
        (
            index,
...
)
{
    ...
    return autoPtr<dfGAMGInterface>(cstrIter()(index, coarseInterfaces, is));
}
