...
#include "cyclicAMIdfGAMGInterface.H"
...
namespace Foam
{
    defineTypeNameAndDebug(cyclicAMIdfGAMGInterface, 0);
    addToRunTimeSelectionTable
    (
        dfGAMGInterface,
        cyclicAMIdfGAMGInterface,
        lduInterface
    );
}
...
Foam::cyclicAMIdfGAMGInterface::cyclicAMIdfGAMGInterface
(
    const label index,
...
Foam::tmp<Foam::labelField> Foam::cyclicAMIdfGAMGInterface::internalFieldTransfer
(
    const Pstream::commsTypes,
    const labelUList& iF
) const
{
    const cyclicAMIdfGAMGInterface& nbr =
        dynamic_cast<const cyclicAMIdfGAMGInterface&>(neighbPatch());
