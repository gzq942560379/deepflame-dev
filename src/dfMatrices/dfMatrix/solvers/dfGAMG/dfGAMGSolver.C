...
namespace Foam
{
    defineTypeNameAndDebug(dfGAMGSolver, 0);

    dfMatrix::solver::addsymMatrixConstructorToTable<dfGAMGSolver>
        adddfGAMGSolverMatrixConstructorToTable_;

    dfMatrix::solver::addasymMatrixConstructorToTable<dfGAMGSolver>
        adddfGAMGAsymSolverMatrixConstructorToTable_;
}


// * * * * * * * * * * * * * * * * Constructors  * * * * * * * * * * * * * * //

Foam::dfGAMGSolver::dfGAMGSolver
(
...
:
    ...
    agglomeration_(dfGAMGAgglomeration::New(matrix_.ldu(), controlDict_)),
    matrixLevels_(agglomeration_.size()),
    dfMatrixLevels_(agglomeration_.size()),
    ...
    interfaceLevelsIntCoeffs_(agglomeration_.size())
{
    ...
    Info << "dfGAMGSolver settings :" << endl
        ...
        forAll(agglomeration_, fineLevelIndex)
        {
            if (agglomeration_.hasMeshLevel(fineLevelIndex))
            {
                ...
                )
                {
                    ...
                    PtrList<dfGAMGInterface> dummyPrimMeshInterfaces
                    (
...
void Foam::dfGAMGSolver::readControls()
{
    dfMatrix::solver::readControls();
    ...
    if (debug)
    {
        Pout<< "dfGAMGSolver settings :" << endl
            << "    cacheAgglomeration:" << cacheAgglomeration_ << endl
            << "    nPreSweeps:" << nPreSweeps_ << endl
            << "    preSweepsLevelMultiplier:" << preSweepsLevelMultiplier_ << endl
            << "    maxPreSweeps:" << maxPreSweeps_ << endl
...
const Foam::lduMatrix& Foam::dfGAMGSolver::matrixLevel(const label i) const
{
    if (i == 0)
    {
        return matrix_.ldu();
...
Foam::dfGAMGSolver::interfaceBouCoeffsLevel
(
    const label i
) const
{
...
const Foam::FieldField<Foam::Field, Foam::scalar>&
Foam::dfGAMGSolver::interfaceIntCoeffsLevel
(
    const label i
) const
{
