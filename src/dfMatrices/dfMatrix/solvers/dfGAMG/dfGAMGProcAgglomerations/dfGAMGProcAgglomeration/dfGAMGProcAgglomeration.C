...
namespace Foam
{
    defineTypeNameAndDebug(dfGAMGProcAgglomeration, 0);
    defineRunTimeSelectionTable(dfGAMGProcAgglomeration, dfGAMGAgglomeration);
}


// * * * * * * * * * * * * Protected Member Functions  * * * * * * * * * * * //

void Foam::dfGAMGProcAgglomeration::printStats
(
    Ostream& os,
...
Foam::labelListList Foam::dfGAMGProcAgglomeration::globalCellCells
(
    const lduMesh& mesh
)
{
...
bool Foam::dfGAMGProcAgglomeration::agglomerate
(
...
Foam::dfGAMGProcAgglomeration::dfGAMGProcAgglomeration
(
    dfGAMGAgglomeration& agglom,
...
Foam::autoPtr<Foam::dfGAMGProcAgglomeration> Foam::dfGAMGProcAgglomeration::New
(
    const word& type,
    dfGAMGAgglomeration& agglom,
    const dictionary& controlDict
)
{
    if (debug)
    {
        InfoInFunction << "Constructing dfGAMGProcAgglomeration" << endl;
    }

    dfGAMGAgglomerationConstructorTable::iterator cstrIter =
        dfGAMGAgglomerationConstructorTablePtr_->find(type);

    if (cstrIter == dfGAMGAgglomerationConstructorTablePtr_->end())
    {
        FatalErrorInFunction
            << "Unknown dfGAMGProcAgglomeration type "
            << type << " for dfGAMGAgglomeration " << agglom.type() << nl << nl
            << "Valid dfGAMGProcAgglomeration types are :" << endl
            << dfGAMGAgglomerationConstructorTablePtr_->sortedToc()
            << exit(FatalError);
    }

    return autoPtr<dfGAMGProcAgglomeration>(cstrIter()(agglom, controlDict));
}
