...
Foam::autoPtr<Foam::dfGAMGInterfaceField> Foam::dfGAMGInterfaceField::New
(
...
)
{
    ...
    if (cstrIter == lduInterfaceFieldConstructorTablePtr_->end())
    {
        FatalErrorInFunction
            << "Unknown dfGAMGInterfaceField type "
            << coupleType << nl
            << "Valid dfGAMGInterfaceField types are :"
            << lduInterfaceFieldConstructorTablePtr_->sortedToc()
            << exit(FatalError);
    }

    return autoPtr<dfGAMGInterfaceField>(cstrIter()(GAMGCp, fineInterface));
}
...
)
{
    ...
    return autoPtr<dfGAMGInterfaceField>(cstrIter()(GAMGCp, doTransform, rank));
}
