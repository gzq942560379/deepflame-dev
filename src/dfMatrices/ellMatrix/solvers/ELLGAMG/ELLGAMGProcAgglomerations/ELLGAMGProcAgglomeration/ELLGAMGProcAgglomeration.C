...
namespace Foam
{
    defineTypeNameAndDebug(ELLGAMGProcAgglomeration, 0);
    defineRunTimeSelectionTable(ELLGAMGProcAgglomeration, ELLGAMGAgglomeration);
}


// * * * * * * * * * * * * Protected Member Functions  * * * * * * * * * * * //

void Foam::ELLGAMGProcAgglomeration::printStats
(
    Ostream& os,
    ELLGAMGAgglomeration& agglom
) const
{
...
Foam::labelListList Foam::ELLGAMGProcAgglomeration::globalCellCells
(
    const lduMesh& mesh
)
{
...
bool Foam::ELLGAMGProcAgglomeration::agglomerate
(
...
Foam::ELLGAMGProcAgglomeration::ELLGAMGProcAgglomeration
(
    ELLGAMGAgglomeration& agglom,
...
Foam::autoPtr<Foam::ELLGAMGProcAgglomeration> Foam::ELLGAMGProcAgglomeration::New
(
    const word& type,
    ELLGAMGAgglomeration& agglom,
    const dictionary& controlDict
)
{
    if (debug)
    {
        InfoInFunction << "Constructing ELLGAMGProcAgglomeration" << endl;
    }

    ELLGAMGAgglomerationConstructorTable::iterator cstrIter =
        ELLGAMGAgglomerationConstructorTablePtr_->find(type);

    if (cstrIter == ELLGAMGAgglomerationConstructorTablePtr_->end())
    {
        FatalErrorInFunction
            << "Unknown ELLGAMGProcAgglomeration type "
            << type << " for ELLGAMGAgglomeration " << agglom.type() << nl << nl
            << "Valid ELLGAMGProcAgglomeration types are :" << endl
            << ELLGAMGAgglomerationConstructorTablePtr_->sortedToc()
            << exit(FatalError);
    }

    return autoPtr<ELLGAMGProcAgglomeration>(cstrIter()(agglom, controlDict));
}
