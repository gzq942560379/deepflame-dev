...
namespace Foam
{
    defineTypeNameAndDebug(ELLGAMGSolver, 0);

    ellMatrix::solver::addsymMatrixConstructorToTable<ELLGAMGSolver>
        addELLGAMGSolverMatrixConstructorToTable_;

    ellMatrix::solver::addasymMatrixConstructorToTable<ELLGAMGSolver>
        addELLGAMGAsymSolverMatrixConstructorToTable_;
}


// * * * * * * * * * * * * * * * * Constructors  * * * * * * * * * * * * * * //

Foam::ELLGAMGSolver::ELLGAMGSolver
(
...
:
    ...
    agglomeration_(ELLGAMGAgglomeration::New(matrix_.ldu(), controlDict_)),
    matrixLevels_(agglomeration_.size()),
    ellMatrixLevels_(agglomeration_.size()),
    ...
    interfaceLevelsIntCoeffs_(agglomeration_.size())
{
    ...
    Info << "ELLGAMGSolver settings :" << endl
        ...
        forAll(agglomeration_, fineLevelIndex)
        {
            if (agglomeration_.hasMeshLevel(fineLevelIndex))
            {
                ...
                )
                {
                    ...
                    PtrList<ELLGAMGInterface> dummyPrimMeshInterfaces
                    (
                    ...
                    forAll(fineMeshInterfaces, intI)
                    {
                        ...
                        {
                            OStringStream os;
                            refCast<const ELLGAMGInterface>
                            (
...
void Foam::ELLGAMGSolver::readControls()
{
    ellMatrix::solver::readControls();
    ...
    if (debug)
    {
        Pout<< "ELLGAMGSolver settings :" << endl
            << "    cacheAgglomeration:" << cacheAgglomeration_ << endl
            << "    nPreSweeps:" << nPreSweeps_ << endl
            << "    preSweepsLevelMultiplier:" << preSweepsLevelMultiplier_ << endl
            << "    maxPreSweeps:" << maxPreSweeps_ << endl
...
const Foam::lduMatrix& Foam::ELLGAMGSolver::matrixLevel(const label i) const
{
    if (i == 0)
    {
        return matrix_.ldu();
...
const Foam::lduInterfaceFieldPtrsList& Foam::ELLGAMGSolver::interfaceLevel
(
    const label i
) const
{
...
Foam::ELLGAMGSolver::interfaceBouCoeffsLevel
(
    const label i
) const
{
...
const Foam::FieldField<Foam::Field, Foam::scalar>&
Foam::ELLGAMGSolver::interfaceIntCoeffsLevel
(
    const label i
) const
{
