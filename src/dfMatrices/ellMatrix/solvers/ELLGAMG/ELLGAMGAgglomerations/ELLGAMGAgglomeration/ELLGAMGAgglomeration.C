...
#include "lduMatrix.H"
#include "Time.H"
#include "ELLGAMGInterface.H"
#include "ELLGAMGProcAgglomeration.H"
#include "pairELLGAMGAgglomeration.H"
#include "IOmanip.H"
...
namespace Foam
{
    defineTypeNameAndDebug(ELLGAMGAgglomeration, 0);
    defineRunTimeSelectionTable(ELLGAMGAgglomeration, lduMesh);
    defineRunTimeSelectionTable(ELLGAMGAgglomeration, lduMatrix);
    defineRunTimeSelectionTable(ELLGAMGAgglomeration, geometry);
}


// * * * * * * * * * * * * Protected Member Functions  * * * * * * * * * * * //

void Foam::ELLGAMGAgglomeration::compactLevels(const label nCreatedLevels)
{
...
bool Foam::ELLGAMGAgglomeration::continueAgglomerating
(
...
Foam::ELLGAMGAgglomeration::ELLGAMGAgglomeration
(
    const lduMesh& mesh,
    const dictionary& controlDict
)
:
    MeshObject<lduMesh, Foam::GeometricMeshObject, ELLGAMGAgglomeration>(mesh),
    maxLevels_(
    ...
    (
      ...
      ? ELLGAMGProcAgglomeration::New
        (
            controlDict.lookup("processorAgglomerator"),
            *this,
            controlDict
        )
      : autoPtr<ELLGAMGProcAgglomeration>(nullptr)
    ),
...
const Foam::ELLGAMGAgglomeration& Foam::ELLGAMGAgglomeration::New
(
    const lduMesh& mesh,
    const dictionary& controlDict
)
{
    if
    (
        !mesh.thisDb().foundObject<ELLGAMGAgglomeration>
        (
            ELLGAMGAgglomeration::typeName
        )
    )
    {
        ...
        (
            controlDict,
            "geometricELLGAMGAgglomerationLibs",
        ...
        if (cstrIter == lduMeshConstructorTablePtr_->end())
        {
            FatalErrorInFunction
                << "Unknown ELLGAMGAgglomeration type "
                << agglomeratorType << ".\n"
                << "Valid matrix ELLGAMGAgglomeration types are :"
                << lduMatrixConstructorTablePtr_->sortedToc() << endl
                << "Valid geometric ELLGAMGAgglomeration types are :"
    ...
    {
        return mesh.thisDb().lookupObject<ELLGAMGAgglomeration>
        (
            ELLGAMGAgglomeration::typeName
        );
    }
...
)
{
    ...
    )
    {
        ...
        (
            controlDict,
            "algebraicELLGAMGAgglomerationLibs",
...
Foam::autoPtr<Foam::ELLGAMGAgglomeration> Foam::ELLGAMGAgglomeration::New
(
    const lduMesh& mesh,
...
)
{
    ...
    return autoPtr<ELLGAMGAgglomeration>
    (
        cstrIter()
        (
            mesh,
...
const Foam::lduMesh& Foam::ELLGAMGAgglomeration::meshLevel
(
    const label i
) const
{
...
bool Foam::ELLGAMGAgglomeration::hasMeshLevel(const label i) const
{
    if (i == 0)
    {
        return true;
...
const Foam::lduInterfacePtrsList& Foam::ELLGAMGAgglomeration::interfaceLevel
(
    const label i
) const
{
...
void Foam::ELLGAMGAgglomeration::clearLevel(const label i)
{
    if (hasMeshLevel(i))
    {
...
const Foam::labelList& Foam::ELLGAMGAgglomeration::procAgglomMap
(
    const label leveli
) const
{
...
const Foam::labelList& Foam::ELLGAMGAgglomeration::agglomProcIDs
(
    const label leveli
) const
{
...
bool Foam::ELLGAMGAgglomeration::hasProcMesh(const label leveli) const
{
    return procCommunicator_[leveli] != -1;
}


Foam::label Foam::ELLGAMGAgglomeration::procCommunicator(const label leveli) const
{
    return procCommunicator_[leveli];
}


const Foam::labelList& Foam::ELLGAMGAgglomeration::cellOffsets
(
    const label leveli
) const
{
...
const Foam::labelListList& Foam::ELLGAMGAgglomeration::faceMap
(
    const label leveli
) const
{
...
const Foam::labelListList& Foam::ELLGAMGAgglomeration::boundaryMap
(
    const label leveli
) const
{
...
const Foam::labelListListList& Foam::ELLGAMGAgglomeration::boundaryFaceMap
(
    const label leveli
) const
{
...
bool Foam::ELLGAMGAgglomeration::checkRestriction
(
