...
#include "processorELLGAMGInterface.H"

// * * * * * * * * * * * * * * * Member Functions  * * * * * * * * * * * * * //

void Foam::ELLGAMGAgglomeration::agglomerateLduAddressing
(
...
)
{
    ...
    forAll(fineInterfaces, inti)
    {
        if (fineInterfaces.set(inti))
        {
            ...
            patchFineToCoarse[inti] = refCast<const ELLGAMGInterface>
            (
    ...
    (
        coarseInterfaces,
        lduPrimitiveMesh::nonBlockingSchedule<processorELLGAMGInterface>
        (
            coarseInterfaces
        )
    );
...
void Foam::ELLGAMGAgglomeration::procAgglomerateLduAddressing
(
...
void Foam::ELLGAMGAgglomeration::procAgglomerateRestrictAddressing
(
    const label comm,
    const labelList& procIDs,
    const label levelIndex
...
void Foam::ELLGAMGAgglomeration::combineLevels(const label curLevel)
{
    ...
    forAll(prevInterLevel, inti)
    {
        ...
        {
            ELLGAMGInterface& prevInt = refCast<ELLGAMGInterface>
            (
            ...
            const ELLGAMGInterface& curInt = refCast<const ELLGAMGInterface>
            (
                curInterLevel[inti]
            );
...
//void Foam::ELLGAMGAgglomeration::gatherList
//(
//    const label comm,
//    const labelList& procIDs,
//
...
void Foam::ELLGAMGAgglomeration::calculateRegionMaster
(
    const label comm,
    const labelList& procAgglomMap,
