...
#include "ELLGAMGInterfaceField.H"
#include "processorLduInterfaceField.H"
#include "processorELLGAMGInterfaceField.H"

// * * * * * * * * * * * * * Private Member Functions  * * * * * * * * * * * //

void Foam::ELLGAMGSolver::agglomerateMatrix
(
...
)
{
    ...
    if (UPstream::myProcNo(fineMatrix.mesh().comm()) != -1)
    {
        ...
        // Set the coarse level matrix
        ellMatrixLevels_.set
        (
            fineLevelIndex,
            new ellMatrix(coarseMatrix)
        );
...
void Foam::ELLGAMGSolver::agglomerateInterfaceCoefficients
(
    const label fineLevelIndex,
    const lduInterfacePtrsList& coarseMeshInterfaces,
    PtrList<lduInterfaceField>& coarsePrimInterfaces,
...
) const
{
    ...
    forAll(fineInterfaces, inti)
    {
        ...
        {
            const ELLGAMGInterface& coarseInterface =
...
void Foam::ELLGAMGSolver::gatherMatrices
(
...
) const
{
    if (debug)
    {
        Pout<< "ELLGAMGSolver::gatherMatrices :"
...
void Foam::ELLGAMGSolver::procAgglomerateMatrix
(
            ...
            forAll(bMap, procIntI)
            {
                ...
                if (allIntI != -1)
                {
                    ...
                    if (!allInterfaces.set(allIntI))
                    {
                        ...
                        if (proci == 0)
                        {
                            const processorELLGAMGInterfaceField& procInt =
                                refCast
                                <
                                    const processorELLGAMGInterfaceField
                                >
                                (
