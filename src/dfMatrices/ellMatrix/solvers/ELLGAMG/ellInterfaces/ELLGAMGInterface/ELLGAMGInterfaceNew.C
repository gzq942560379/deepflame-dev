...
Foam::autoPtr<Foam::ELLGAMGInterface> Foam::ELLGAMGInterface::New
(
    const label index,
...
)
{
    ...
    if (cstrIter == lduInterfaceConstructorTablePtr_->end())
    {
        FatalErrorInFunction
            << "Unknown ELLGAMGInterface type " << coupleType << ".\n"
            << "Valid ELLGAMGInterface types are :"
            << lduInterfaceConstructorTablePtr_->sortedToc()
            << exit(FatalError);
    }

    return autoPtr<ELLGAMGInterface>
    (
        cstrIter()
        (
            index,
...
)
{
    ...
    return autoPtr<ELLGAMGInterface>(cstrIter()(index, coarseInterfaces, is));
}
