...
class Net(torch.nn.Module):
    ...
    def write_linear(self, dir, layer_id, file_id):

        weight_array = self.layers[layer_id].weight.detach().numpy()
        with open(os.path.join(dir, "linear_{}_weights_rowmajor_{}_{}.data".format(file_id,weight_array.shape[1],weight_array.shape[0])), "wb") as f:
            for c in range(weight_array.shape[1]):
                for r in range(weight_array.shape[0]):
                    f.write(weight_array[r,c])

        bias_array = self.layers[layer_id].bias.detach().numpy()
        with open(os.path.join(dir, "linear_{}_bias_{}.data".format(file_id,bias_array.shape[0])), "wb") as f:
            for r in range(bias_array.shape[0]):
                f.write(bias_array[r])

    def write_model(self, dir):
        if not os.path.exists(dir):
            os.makedirs(dir)
        self.write_linear(dir, 0, 0)
        self.write_linear(dir, 2, 1)
        self.write_linear(dir, 4, 2)
        self.write_linear(dir, 6, 3)
...
if __name__ == "__main__":
    ...
    model0.write_model("model/0")
    model1.write_model("model/1")
    model2.write_model("model/2")
